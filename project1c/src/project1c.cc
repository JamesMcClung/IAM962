...
// This program solves the inviscid Burger's equation,
//   u_t + u * u_x = 0, using an Adams-Bashforth (AB) method in time and
//   central difference in space.

// AB methods solve u_t = F(u) explicitly. For Burger's equation,
//   F(u) = -u * d/dx[u]. We replace the d/dx operator by the
...
static constexpr real b = dt / (2 * dx);
...
void solve_for_du(const u_type &u, u_type &du) {
    for (int i = 1; i < nx - 1; i++) {
        du(i, 0) = -b * u(i, 0) * (u(i + 1, 0) - u(i - 1, 0));
    }
    // periodic BCs
    du(0, 0) = -b * u(0, 0) * (u(1, 0) - u(nx - 1, 0));
    du(nx - 1, 0) = -b * u(nx - 1, 0) * (u(0, 0) - u(nx - 2, 0));
}
...
void write_params() {
    linalg::FullMatrix params_mat({{real(nx), real(nt), min_x, max_x, dx, dt, real(write_every), real(which_AB), cfl}});
